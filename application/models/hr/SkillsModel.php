...
class SkillsModel extends CI_Model { 
	...
	public function Create($data) {
		...
		$this->db->insert('hr_general_skills',$data);
		$inserted = $this->db->insert_id();
		if ($inserted > 0) {
			$result = $this->LoadSkillsList($inserted);
	...
	public function Delete($data) {
		...
		$this->db->update('hr_general_skills');
	...
	public function Update($id, $data) {
	    ...
	    $this->db->where('Id', $id);
	    $query = $this->db->update('hr_general_skills', $data);
		$update = $this->db->affected_rows();
		if ($update > 0) {
			$result = $this->LoadSkillsList($id);
			if ($result->num_rows() > 0) {
				return $result->result();
			}
			return $data;
